...
``0f289e4``), ported to released ``colpali-engine`` v0.3.16: the PR's
``COLPALI_SCORES_BACKEND`` dispatcher does not exist in any release, so vanilla vs LIK
is toggled with ``--variant`` (``lik`` calls :func:`patch_colpali_engine` first) and the
instrumentation wraps the loss head's ``forward`` instead of a ``maxsim_inbatch``
dispatcher. Each wrapped call records:
