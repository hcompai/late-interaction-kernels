...
helpers, PyLate patching, and research kernels, install the PyPI package
directly (`pip install late-interaction-kernels`).
...
scores = lik.maxsim_varlen_inference(
    Q, D_packed, cu_seqlens_d=cu_seqlens_d, normalize=True,
)
...
| `maxsim_varlen_inference`    | Packed-tensor layout with `cu_seqlens`.                |
...
| `maxsim_residual_inference`  | Fused PLAID / ColBERTv2 decompress + MaxSim.            |
...
`MaxSimScorer`), experimental variants (`soft`, `smooth`, `matryoshka`,
`xtr`) and PyLate compat stay PyPI-only.
...
│       └── *.py                             # symlinks into ../../late_interaction_kernels/
