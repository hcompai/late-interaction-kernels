...
# The CUDA marker gates the whole module: without a GPU we never even try to
# shadow-import the torch-ext tree (which transitively requires Triton).
...
def test_maxsim_inference_parity(hf_lik, rel):
    from late_interaction_kernels.reference import maxsim_reference  # PyPI side, ground truth
...
def test_plaid_exports_callable(hf_lik):
    """``maxsim_residual_*`` and ``plaid_approx_score`` are reachable on the HF tree.

    Numerical parity for PLAID is already covered by ``tests/test_plaid.py`` against
    the PyPI package; this test is a smoke check that the export reaches the right
    implementation in the HF-facing tree.
    """
    assert callable(hf_lik.maxsim_residual)
