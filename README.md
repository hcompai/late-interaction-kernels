...
GPU tests run on AWS CodeBuild (A10G). To run them on a PR, apply the `run-gpu-tests` label — this requires repository admin permissions, so ping a maintainer if your PR needs it. Maintainers can also trigger an on-demand run via the [GPU CI workflow](https://github.com/hcompai/late-interaction-kernels/actions/workflows/gpu-ci.yml) `workflow_dispatch`.
